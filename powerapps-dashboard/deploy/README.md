...
SharePoint  ──►  Power BI    ──►  Build Percy-Query ─►  Create the     ──►  Build         ──►  Test
...
> a phase here. The full step detail lives in [`DEPLOY.md`](DEPLOY.md); this page is the map + order.
...
### Phase 3 — Build `Percy-Query` *(the diagnostic flow — build this FIRST)*
One flow: Copilot trigger (`template`,`ope`,`who`) → `Switch(template)` pastes the approved DAX →
Power BI "Run a query against a dataset" → return `firstTableRows`. **The agent passes a template
key, never DAX.** Build it now so it exists to attach to the agent in Phase 4.
- 📄 [`flows/Percy-Query.build.md`](flows/Percy-Query.build.md) — **the click-by-click build**.
- 📄 [`flows/dax-templates.md`](flows/dax-templates.md) — which template key → which DAX → what to insert.
- 📄 [`PERCY_BUILD_PACK.md` §7](../formulas/percy/backend/PERCY_BUILD_PACK.md#7-the-single-diagnostic-action--percy-query) — action spec + per-template evidence.
...
2. **Add the one action** "Run Percy Diagnostic" = the `Percy-Query` flow, inputs `template`/`ope`/`who`.
   - 📄 [`PERCY_BUILD_PACK.md` §15.2](../formulas/percy/backend/PERCY_BUILD_PACK.md#152-the-one-action--its-description-for-copilot-studio-orchestration) — the action description to give it.
...
SharePoint *item created/modified* → guard `Status=Pending` & empty `AnswerText` → read
`ConversationJson` → **run the published Percy agent** (Phase 4) → write `AnswerText` +
`Status="Answered"` → error branch. **No Power Apps changes.**
- 📄 [`flows/Percy-Orchestrator.build.md`](flows/Percy-Orchestrator.build.md) — **the click-by-click build**.
- 📄 [`PERCY_BUILD_PACK.md` §4](../formulas/percy/backend/PERCY_BUILD_PACK.md#4-main-power-automate-flow-percy-orchestrator) — flow design; [§3](../formulas/percy/backend/PERCY_BUILD_PACK.md#3-power-apps-behaviour) — the (unchanged) Power Apps contract.
...
| 3 | [`flows/Percy-Query.build.md`](flows/Percy-Query.build.md) | Build the **one diagnostic flow** (Switch over template keys). | Phase 3 |
...
| 4 | [`copilot-studio/topics.md`](copilot-studio/topics.md) | **Topics** + the single action to add. | Phase 4.3 |
...
| — | [`flows/README.md`](flows/README.md) | Overview of the **two flows** + connections. | Phase 3 & 5 |
